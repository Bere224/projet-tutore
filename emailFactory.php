...
class emailFactory {
    ...
    public static function emailInscription($utilisateur) {
    	...
    	return new email("Validation du compte Demos.com", "Pour valider votre email, cliquez-<a href=\"http://localhost/projet/projet-tutore/validation.php?id=$id\">ici</a> >", $utilisateur->getMail()); //todo faire mieux
    }
    ...
    public static function emailInvitationGroupe($utilisateur, $groupe) {
    	...
    	$idGrp=$groupe->getId();
    	$nomGrp=$groupe->getNom();
    	return new email("Invitation dans un groupe sur Demos.com", "Pour vous inscrire dans le groupe $nomGrp, cliquez-<a href=\"http://localhost/projet/projet-tutore/newmembre.php?idUtilisateur=$idUt&idGroupe=$idGrp\">ici</a> >", $utilisateur->getMail()); //todo faire mieux
    }
